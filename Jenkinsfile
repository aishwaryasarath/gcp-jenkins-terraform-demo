pipeline {
  ...
  parameters {
    string(name: 'PROJECT_ID', defaultValue: 'your-gcp-project-id', description: 'GCP Project ID')
  ...
  stages {
    stage('Checkout') { steps { checkout scm; sh 'mkdir -p .bin' } }
    ...
    stage('Terraform Plan') {
      steps {
        withCredentials([file(credentialsId: 'gcp-creds', variable: 'GOOGLE_APPLICATION_CREDENTIALS')]) {
          dir('terraform') {
            sh '${TF_BIN} plan -input=false -var "project_id=${PROJECT_ID}" -var "region=${REGION}" -var "credentials_json=${GOOGLE_APPLICATION_CREDENTIALS}" -out /tmp/tfplan'
          }
        }
      }
    }
    stage('Apply?') {
      steps {
        script { input message: 'Apply Terraform?', ok: 'Apply' }
        withCredentials([file(credentialsId: 'gcp-creds', variable: 'GOOGLE_APPLICATION_CREDENTIALS')]) {
          dir('terraform') {
    ...
    stage('Destroy?') {
      ...
      steps {
        withCredentials([file(credentialsId: 'gcp-creds', variable: 'GOOGLE_APPLICATION_CREDENTIALS')]) {
          dir('terraform') {
            sh '${TF_BIN} destroy -auto-approve -var "project_id=${PROJECT_ID}" -var "region=${REGION}" -var "credentials_json=${GOOGLE_APPLICATION_CREDENTIALS}"'
          }
        }
      }
    }
